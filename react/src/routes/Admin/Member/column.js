...
export const columnsMember = ({t,
  memberCategorys,
  handleEditMember, handleDeleteMember, handlePatchMember, checkPermission
}) => {
  return [
    {
    ...
    {
      title: t('routes.Admin.Member.Position'),
      name: 'position',
      tableItem: {
        filter: {type: 'search'},
        sorter: true,
    ...
    },
    {
      ...
      name: 'categories.member_category_id',
      tableItem: {
        filter: {
          type: "checkbox",
          list: convertToFiltersTable(memberCategorys)
        },
    ...
    },
    {
      ...
      formItem: {
        type: "tree_select",
        list: formatTreeSelect(memberCategorys),
        mode: "multiple",
        rules: [
          { type: 'required' },
        ]
    ...
    },
    {
      ...
      tableItem: {
        render: (text, record) => {
          ...
          return (
            <Space size="middle">
              {checkPermission(routerLinks("Member", "api"), "PUT") && (
                <Tooltip title={t('routes.Admin.User.edit')}>
                  <Button onClick={() => handleEditMember(record)} size="small"
              ...
              {checkPermission(routerLinks("Member", "api"), "PUT") && (
                <Tooltip title={t('routes.Admin.User.status')}>
                  <Button
                    onClick={() => handlePatchMember({status: (record.status === 1 ? 0 : 1)}, record.id)}
              ...
              {checkPermission(routerLinks("Member", "api"), "DELETE") && (
...
export const columnsMemberCategory = ({t}) => [
  {
    title: t('routes.Admin.User.name'),
    name: 'name',
    formItem: {
