...
export const columnsStartup = ({t,
  startupCategorys,
  handleEditStartup, handleDeleteStartup, handlePatchStartup, checkPermission
}) => {
  return [
    {
    ...
    },
    {
      ...
      name: 'categories.startup_category_id',
      tableItem: {
        filter: {
          type: "checkbox",
          list: convertToFiltersTable(startupCategorys)
        },
    ...
    },
    {
      ...
      formItem: {
        type: "tree_select",
        list: formatTreeSelect(startupCategorys),
        mode: "multiple",
        rules: [
          { type: 'required' },
        ]
    ...
    },
    {
      ...
      tableItem: {
        render: (text, record) => {
          ...
          return (
            <Space size="middle">
              {checkPermission(routerLinks("Startup", "api"), "PUT") && (
                <Tooltip title={t('routes.Admin.User.edit')}>
                  <Button onClick={() => handleEditStartup(record)} size="small"
              ...
              {checkPermission(routerLinks("Startup", "api"), "PUT") && (
                <Tooltip title={t('routes.Admin.User.status')}>
                  <Button
                    onClick={() => handlePatchStartup({status: (record.status === 1 ? 0 : 1)}, record.id)}
              ...
              {checkPermission(routerLinks("Startup", "api"), "DELETE") && (
                <Popconfirm placement="left" title={t('components.Datatable.areYouSureWant')}
                            onConfirm={() => handleDeleteStartup(record.id)}>
...
export const columnsStartupCategory = ({t}) => [
  {
    title: t('routes.Admin.User.name'),
    name: 'name',
    formItem: {
